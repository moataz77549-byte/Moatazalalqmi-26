...
export async function invalidateCache(pattern?: string): Promise<void> {
  // Cache invalidation is handled by Redis TTL expiration.
  // For pattern-based invalidation, use Redis SCAN in production:
  // const keys = await redisClient.scan(pattern); await redisClient.del(...keys);
  // Currently relying on TTL-based expiration which is sufficient for prompt caching.
}
